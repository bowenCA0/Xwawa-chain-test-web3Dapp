...
document.addEventListener('DOMContentLoaded', function () {
   ...
   * - 根据用户偏好加载对应语言内容
   */
  var langButtons = document.querySelectorAll('.language-switcher button');
  var langElements = document.querySelectorAll('[data-en], [data-zh]');
  langButtons.forEach(function (button) {
    button.addEventListener('click', function () {
      var lang = this.getAttribute('data-lang'); // 更新激活状态的按钮样式

      langButtons.forEach(function (btn) {
        return btn.classList.remove('active');
      });
      this.classList.add('active'); // 根据选择的语言更新页面文本内容

      langElements.forEach(function (el) {
        if (el.hasAttribute("data-".concat(lang))) {
          el.textContent = el.getAttribute("data-".concat(lang));
        }
      }); // 保存用户语言偏好到本地存储

      localStorage.setItem('marketplace-language', lang); // TODO: 发送语言偏好到后端API
  ...
  buyButtons.forEach(function (button) {
    button.addEventListener('click', function () {
      ...
      var productPrice = this.getAttribute('data-product-price'); // 计算XWAWA代币数量 (简单汇率转换)
      // TODO: 从API获取实时汇率

      var tokenAmount = Math.round(parseFloat(productPrice) * 10); // 更新模态框中的商品详情

      document.getElementById('modal-product-name').textContent = productName;
      document.getElementById('modal-product-price').textContent = productPrice;
      document.getElementById('modal-token-amount').textContent = tokenAmount; // 存储商品ID用于后续订单创建

      modal.setAttribute('data-current-product-id', productId); // 显示支付模态框

      modal.style.display = 'block'; // 重置模态框状态
  ...
  createOrderBtn.addEventListener('click', function () {
    ...
    var productId = modal.getAttribute('data-current-product-id'); // 调用订单创建函数
  ...
  copyAddressBtn.addEventListener('click', function () {
    ...
    document.execCommand('copy'); // 显示复制成功提示

    this.textContent = '已复制!';
    setTimeout(function () {
      _this.textContent = '复制';
    }, 2000);
  });
  /**
   * 邮箱格式验证函数
  ...
  function resetModal() {
    ...
    checkPaymentBtn.classList.add('hidden'); // 重置按钮文本
  ...
  function createOrder(email, productId) {
    ...
    var productPrice = document.getElementById('modal-product-price').textContent;
    var tokenAmount = document.getElementById('modal-token-amount').textContent; // 显示加载状态

    createOrderBtn.textContent = '创建订单中...';
    createOrderBtn.disabled = true; // TODO: 替换为实际的API调用
    // const orderData = {
    //     productId: productId,
    //     customerEmail: email,
    ...
    setTimeout(function () {
      // 生成模拟订单数据 (实际应从API返回)
      var orderId = 'XW' + Math.floor(Math.random() * 1000000).toString().padStart(6, '0');
      var walletAddress = generatePaymentAddress(); // 实际应从后端生成
      // 更新UI显示订单详情

      document.getElementById('order-id').textContent = orderId;
      document.getElementById('wallet-address').value = walletAddress;
      document.getElementById('payment-token-amount').textContent = tokenAmount; // 生成支付二维码

      generatePaymentQRCode(walletAddress); // 启动订单过期倒计时

      startExpiryCountdown(); // 更新UI状态：显示支付信息，隐藏创建按钮

      paymentInfo.classList.remove('hidden');
      createOrderBtn.classList.add('hidden');
      checkPaymentBtn.classList.remove('hidden'); // 重置按钮状态
  ...
  function generatePaymentQRCode(walletAddress) {
    if (window.QRCode) {
      var qrCodeElement = document.getElementById('payment-qr-code');
  ...
  function checkPaymentStatus() {
    ...
    var expectedAmount = document.getElementById('payment-token-amount').textContent; // 显示检查状态

    checkPaymentBtn.textContent = '检查中...';
    checkPaymentBtn.disabled = true; // TODO: 替换为实际的API调用
  ...
  function startExpiryCountdown() {
    var minutes = 15; // 订单有效期15分钟

    var seconds = 0;
    var expiryElement = document.getElementById('expiry-time');
    var countdownInterval = setInterval(function () {
      // 检查是否已过期
      if (minutes === 0 && seconds === 0) {
        clearInterval(countdownInterval);
        expiryElement.textContent = '已过期';
        expiryElement.style.color = 'red'; // TODO: 过期处理
        // - 禁用支付按钮
        // - 显示重新创建订单选项
        // - 通知后端释放支付地址
      ...
      expiryElement.textContent = "".concat(displayMinutes, ":").concat(displaySeconds); // 最后5分钟显示警告颜色
...
//# sourceMappingURL=marketplace.dev.js.map
